...
import Web.Twitter.Types.Lens
  (Status, User, statusUser, userScreenName, statusId
  , userName, statusCreatedAt, statusFavoriteCount, statusRetweetCount
  , statusEntities, statusText, statusExtendedEntities
  , statusRetweetedStatus, statusInReplyToStatusId
...
import Data.Time.Format 
  (TimeLocale, formatTime, defaultTimeLocale)
...
import Data.Time.LocalTime 
  (TimeZone, utcToZonedTime, getCurrentTimeZone)
import Data.Set qualified as S
import Text.Pandoc.Definition (Pandoc (..))
import Text.Pandoc.Builder 
  ( link, text, Blocks, Inlines, linebreak,
  header, image, para, doc, space, emph
  )
...
flatSequence :: 
  (Traversable t, Applicative f, Monoid m) =>
  t (f m) -> f m
flatSequence = fmap (foldMap id) . sequenceA
